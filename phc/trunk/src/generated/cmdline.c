/*
  ...
  /opt/gengetopt/bin/gengetopt --unamed-opts --input=src/generated_src/phc.ggo --file-name=src/generated/cmdline 
  ...
  while (1)
    {
      ...
      switch (c)
        ...
        case 0:	/* Long option with no short option */
          ...
          {
            if (local_args_info.dump_php_given)
              {
                fprintf (stderr, "%s: `--dump-php' option given more than once%s\n", argv[0], (additional_error ? additional_error : ""));
                goto failure;
              }
            if (args_info->dump_php_given && ! override)
              continue;
          ...
          {
            if (local_args_info.dump_ast_dot_given)
              {
                fprintf (stderr, "%s: `--dump-ast-dot' option given more than once%s\n", argv[0], (additional_error ? additional_error : ""));
                goto failure;
              }
            if (args_info->dump_ast_dot_given && ! override)
              continue;
            local_args_info.dump_ast_dot_given = 1;
            args_info->dump_ast_dot_given = 1;
            args_info->dump_ast_dot_flag = !(args_info->dump_ast_dot_flag);
          }
          /* Dump the AST from the source in XML format.  */
          else if (strcmp (long_options[option_index].name, "dump-ast-xml") == 0)
          {
            if (local_args_info.dump_ast_xml_given)
              {
                fprintf (stderr, "%s: `--dump-ast-xml' option given more than once%s\n", argv[0], (additional_error ? additional_error : ""));
                goto failure;
              }
            if (args_info->dump_ast_xml_given && ! override)
              continue;
            local_args_info.dump_ast_xml_given = 1;
